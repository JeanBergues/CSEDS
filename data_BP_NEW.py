...
def create_panel_data():
    ...
    result_df = pd.DataFrame(columns=teams, index=range(int(max(df["RoundNO"])+1)))

    # Iterate over rounds and teams to fill in the goals
    for round_num in sorted(df['RoundNO'].unique()):
        round_data = df[df['RoundNO'] == round_num]
    ...
    result_df.to_csv("panel_data_FIX.csv")
