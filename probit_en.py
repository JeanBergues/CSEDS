...
def main():
    ...
    if ESTIMATE_INITIAL_GAMMA:
        constraint = opt.LinearConstraint(np.concatenate((np.array([0, 0]), np.ones(20))).T, 0, 0)
        k_start = np.array([1, 1])
        gamma_start = np.repeat(0, 20)
        ...
        output.to_csv("en_initial_gamma_probit_final.csv")
        result = result.x
    else:
        df = pd.read_csv("en_initial_gamma_probit_final.csv")
    ...
    if ESTIMATE_OVER_ALL_DATA:
        ...
        gamma_init = np.concatenate((np.array(result[2:]), np.zeros(N - 20)))
        train_fully_data = usable_data.to_numpy(dtype=np.int16)
        bounds = opt.Bounds(np.array([-2, -2, -.5, -.5]), np.array([2, 2, .7, .7]))
        all_data_result = opt.minimize(calculate_ll, x0, args=(gamma_init, train_fully_data), tol=1e-2, bounds=bounds)
        print(all_data_result)
        output = pd.DataFrame()
        output["Value"] = all_data_result.x
        output.to_csv("en_full_probit_estimates_final.csv")
        all_data_result = all_data_result.x
    else:
        df = pd.read_csv("en_full_probit_estimates_final.csv")
        all_data_result = np.array(df["Value"])

    FORECAST_ALL_DATA = True
    if FORECAST_ALL_DATA:
        train_fully_data = usable_data.to_numpy(dtype=np.int16)
        output = forecast_f(all_data_result, np.concatenate((np.array(result[2:]), np.zeros(N - 20))), train_fully_data)[0]
        df_output = pd.DataFrame(output, columns = ["Outcome", "PowerH", "PowerA", "ProbA", "ProbD", "ProbH", "Prediction"])
        print(output)
        df_output.to_csv("predictions/en_probit_full_final.csv")
        ...
        df_for_NN.to_csv("en_probit_full_NN_final.csv")
    ...
    if FORECAST_PER_SEASON:
        ...
        for s in np.unique(test_data[:,3]):
            ...
            parameters = opt.minimize(calculate_ll, x0, args=(gamma_init, train_data), tol=1e-2, bounds=bounds).x
            # USE PARAMETERS
        ...
        df_output = pd.DataFrame(output, columns = ["Outcome", "PowerH", "PowerA", "ProbA", "ProbD", "ProbH", "Prediction"])
        print(output)
        df_output.to_csv("predictions/en_probit_per_season_final.csv")
